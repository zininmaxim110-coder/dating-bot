...
# Глобальные переменные для импорта в app.py
bot = Bot(token=BOT_TOKEN)
storage = MemoryStorage()
dp = Dispatcher(bot, storage=storage)

# ... весь ваш код с хендлерами ...
...
# Этот блок для локального запуска
...
async def main():
   ...
   # Удалите старый блок запуска и добавьте это:
async def start_bot():
    try:
        logger.info("🤖 Бот запущен!")
        await dp.start_polling()
...
# Инициализация всех хендлеров (ваш существующий код до запуска)
# ... весь ваш код с хендлерами ...

if __name__ == '__main__':
    # Запуск поллинга для локальной разработки
